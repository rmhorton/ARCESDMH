...
data <- append(cost_data, effectiveness_data)
...
```{r stochastic_cost_data}

uncertainty <- c(sdloswd=3.5, sdlosnwd=7.1)
...
with(stochastic_data, {
  ...
  curve(dnorm(x, mean=mnloswd, sd=sdloswd),
        from=0, to=21,
        xlab = "Length of stay",
        ylab = "Density",
        main = "Prior distribution for length of stay, wound infection");
  
  curve(dnorm(x, mean=mnlosnwd, sd=sdlosnwd),
        from=0, to=21,
        xlab = "Length of stay",
        ylab = "Density",
...
```
```{r coda_samples}
my_vars <- c("p1", "p2", "diff.QALYs", "diff.cost", "cst.trt", "cst.ctl")
coda_sample <- coda.samples(economic_results$model, variable.names = my_vars, 
    n.iter = 10000)
```
```{r plot_coda_data}
