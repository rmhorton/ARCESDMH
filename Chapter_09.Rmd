...
<font color="#FF0000">
None of these examples currently run in JAGS. The first 3 use the `rank` function, which is parameterized differently in JAGS than in WinBUGS. The fourth example has multi-dimensional data that I have not figured out how to import.

[From Andrew Millard, via the BUGS listserv](https://stackoverflow.com/questions/26370388/use-of-rank-function-in-bugs):
...
for (k in 1:4) { rk[k] <- 5 - rank(T[],k)
                    best[k] <- equals(rk[k],1)}

#All pairwise log odds ratios and odds ratios
for (c in 1:3) { for (k in (c+1):4) { lor[c,k] <- d[k] - d[c]
		                                       log(or[c,k]) <- lor[c,k]
		                                    }
	              }
...
See [this stackoverflow post](https://stackoverflow.com/questions/55628471/rank-function-in-jags) about the rank function in JAGS.
...
<font color="f00"?
NOTE: This example does not use the rank function, but it does have multi-dimensional data that I am not sure how to import
</font>

```{r mtc_re_multiarm, eval=FALSE}
...
data <- "r[,1]  n[,1]	r[,2]	n[,2]	r[,3]	n[,3]	t[,1]  t[,2] t[,3]   na[]   
