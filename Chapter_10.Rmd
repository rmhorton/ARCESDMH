...
<font color="#FF0000">
NOTE: Not all the examples for this chapter are working yet. Chunks that do not even run sucessfully are marked by `eval=FALSE`. Those not explicitly noted as matching the results in the text or winBUGS have not been checked against those sources.
...
model {
  ...
  # Random effects model for logs of study-specific rates
  # lamda1 of leaving state 1
  for (s in 1:15) {
...
<font color="#FF0000"><pre>RUNTIME ERROR:
Compilation error on line 70.
Attempt to redefine node n[1]</pre></font>

I fixed that by moving one line of code to a data block. Now I get:
```
